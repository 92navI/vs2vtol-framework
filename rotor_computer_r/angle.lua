...
function getAngleNoThrust(angle_coef, pitch_coef, roll_coef, cfg)
    local fpitch = angle_coef * pitch_coef * (cfg.MAX_FLAP_ANGLE)
    local froll = (1 - math.abs(angle_coef)) * roll_coef * (cfg.ROLL_ANGLE)
    return fpitch + froll
end
...
function calculateBladeAngle(angle_coef, pitch_coef, roll_coef, thrust, cfg)
    ...
    local fthrust = (1 - math.abs(angle_coef)) * thrust * cfg.THRUST_ANGLE
